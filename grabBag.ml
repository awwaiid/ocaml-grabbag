 ...
 * care which one comes out (so long as its no longer in). Actually I should be
 * more specific; when you pull something out you want it to be pulled out _at
 * random_.
 *
 ...
 * History/Notes
 *   2003.11.10 (rbw)
 *     - Did some touch-ups so that I could show people
 *   2003.04.01 (rbw)
...
let rec grab' bag offset n =
  ...
  | Node(left_sum, right_sum, left, right) ->
    ...
    if n < (offset + left_sum) then
      (* Now we just go traipsing through the left side looking for it *)
    ...
    (* There were no nodes at all, so thats a problem for whoever wanted one *)
...
let rec add' bag v =
  match bag with
    (* If we are down to a leaf lets split it to the right and add the new one
       to the left, becoming an inner-node *)
  | Leaf(_) as original ->
